...
def get_infomation():
    ...
    try:
        ...
        invit_id = info_data.get("invit_id")
        #  根据open_id查询数据
        invitingperson = InvitingPerson.query.filter_by(open_id=open_id).first()
        personopens = PersonOpen.query.filter_by(inperson_id=open_id, invit_id=invit_id, delete=False).all()
...
@api.route('/users/Defect', methods=['POST'])    # 功能为写入文档
def del_visitors():
    """
      访客删除
    :return:
